...
import type {
  PlaytestCommand,
  ZoneTarget,
} from "../../domain/playtest/commands";
import type {
  InstanceId,
  PlaytestState,
  Vec2,
} from "../../domain/playtest/types";
...
type DragSource =
  | { kind: "deck"; instanceId: InstanceId }
...
function Table({
  ...
  const defs = useMemo(
    () => new Map<CardId, CardDefinition>(cards.map((c) => [c.id, c])),
    [cards],
  );
  const view = (id: InstanceId): VisibleCard =>
    viewCard(state, (d) => defs.get(d), id);
  ...
  const [dialog, setDialog] = useState<
    "deck" | "menu" | "gestures" | "reset" | "values" | null
  >(null);
  ...
  function beginDrag(source: DragSource, start: Point, offset: Point) {
    const onCanvas =
      source.kind === "card" &&
      state.instances[source.instanceId]?.zone === "canvas";
    dragRef.current = {
      id: ++dragCounter,
      source,
      point: start,
      offset,
      scale: onCanvas ? zoom.view.current.zoom : 1,
    };
  ...
  function snapBack() {
    ...
    const origin =
      d.source.kind === "deck"
        ...
        : document.querySelector<HTMLElement>(
            `[data-card-id="${d.source.instanceId}"]`,
          );
    if (!ghost || !origin) {
      setDrag(null);
      return;
    ...
    setTimeout(
      () => setDrag((cur) => (cur?.id === d.id ? null : cur)),
      SNAP_BACK_MS,
    );
  }
  ...
  function endDrag(p: Point) {
    ...
    if (target === "canvas") {
      const position = toCanvasPos(
        centre,
        inst.zone === "canvas" && inst.tapped,
      );
      cmd =
        inst.zone === "canvas"
  ...
  function toCanvasPos(centre: Point, tapped: boolean): Vec2 {
    ...
    return {
      x: clamp(
        ((centre.x - r.left) / r.width - v.x) / v.zoom,
        halfW / r.width,
        1 - halfW / r.width,
      ),
      y: clamp(
        ((centre.y - r.top) / r.height - v.y) / v.zoom,
        halfH / r.height,
        1 - halfH / r.height,
      ),
    };
  ...
  function handIndexAt(x: number, excludeId: InstanceId) {
    let index = 0;
    for (const el of handEl.current?.querySelectorAll<HTMLElement>(
      "[data-card-id]",
    ) ?? []) {
  ...
  const cardActions = (
    ...
    onDoubleTap:
      where === "canvas"
        ? () => dispatch({ type: "toggleTapped", instanceId: id })
        : undefined,
    ...
    canStartDrag:
      where === "hand"
        ? (start, point) =>
            !handScrollable ||
            Math.abs(point.y - start.y) > Math.abs(point.x - start.x)
        : undefined,
    onDragStart: (start, offset) =>
      beginDrag({ kind: "card", instanceId: id }, start, offset),
    onDragMove: moveDrag,
    onDragEnd: endDrag,
  ...
  const handGap =
    hand.length < 2
      ...
      : Math.max(
          -0.4 * w,
          Math.min(8, (handWidth - 24 - hand.length * w) / (hand.length - 1)),
        );
  useLayoutEffect(() => {
    const el = handEl.current;
  ...
  const switchPlayer = (delta: number) => {
    ...
    dispatch({
      type: "selectPlayer",
      playerId: (playerId + delta + playerCount) % playerCount,
    });
  };
  ...
  const canvasGestures = useGestures({
    onDragStart: ({ start }) =>
      (swipeStart.current = zoom.pinched.current ? null : start),
    onDragEnd: (p) => {
  ...
  return (
    <div
      className="table"
      style={{ "--lp-ms": `${config.longPressMs}ms` } as CSSProperties}
    >
      <header className="table-bar">
        ...
        <div className="table-title">
          Player {playerId + 1}
          {playerCount > 1 && (
            <span className="table-subtitle"> of {playerCount}</span>
          )}
        </div>
        <button
          className="btn"
      ...
      <div className="table-main">
        ...
        >
          <div
            key={playerId}
            className={`canvas-layer${slide ? ` slide-${slide}` : ""}`}
          >
            <div className="canvas-zoom" ref={zoom.layerRef}>
              ...
              {Object.keys(state.instances).length === 0 && (
                <p className="canvas-hint">
                  No enabled cards were in the list when this playtest started.
                  Add cards in Card Edit, then use Reset Playtest.
                </p>
              )}
            </div>
          </div>
        ...
        <div className="corner">
          <div className="piles">
            <PileZone
              ...
              onTap={() => setOpenPile("graveyard")}
            />
            <PileZone
              pile="exile"
              count={player.zones.exile.length}
              onTap={() => setOpenPile("exile")}
            />
          </div>
          <DeckZone
            ...
            onDragStart={(start, offset) => {
              ...
              if (top)
                beginDrag({ kind: "deck", instanceId: top }, start, offset);
            }}
            onDragMove={moveDrag}
            onDragEnd={endDrag}
            onDragCancel={snapBack}
      ...
      {drag && ghostPoint && (
        <div
          ...
          style={{
            transform: ghostTransform(
              {
                x: ghostPoint.x + drag.offset.x,
                y: ghostPoint.y + drag.offset.y,
              },
        ...
        >
          ...
          {drag.source.kind === "deck" ? (
            <CardView faceUp={false} />
          ) : (
            <VisibleCardView card={view(drag.source.instanceId)} />
          )}
        </div>
      )}
        ...
        <PileListDialog
          ...
          onMove={(id, to) => {
            if (to.zone === "canvas") {
              ...
              to = {
                zone: "canvas",
                position: { x: 0.45 + k * 0.03, y: 0.45 + k * 0.03 },
              };
            }
      ...
      {deckDrop && deckDropCard && (
        <Modal onClose={() => setDeckDrop(null)}>
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Put card into deck"
          >
            <h3>
              Put{" "}
              {deckDropCard.kind === "revealed"
                ? deckDropCard.def.name || "this card"
                : "this card"}{" "}
              into the deck
            </h3>
      ...
      {dialog === "deck" && (
        <Modal onClose={() => setDialog(null)}>
          ...
          >
            <h3>Deck</h3>
            <p>
              {player.zones.deck.length} card
              {player.zones.deck.length === 1 ? "" : "s"} left.
            </p>
            <div className="dialog-actions dialog-actions-stack">
              <button
                className="btn btn-big btn-primary"
      ...
      {dialog === "gestures" && (
        <GestureSettings onClose={() => setDialog(null)} />
      )}
      {dialog === "values" && (
        <PlayerValuesDialog
          playerId={playerId}
...
export function ResetPlaytestDialog({
  onDone,
}: {
  onDone: (didReset: boolean) => void;
}) {
  return (
    <ConfirmDialog
